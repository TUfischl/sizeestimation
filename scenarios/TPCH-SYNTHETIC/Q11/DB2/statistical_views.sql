...
FROM DB2ADMIN.partsupp AS P, DB2ADMIN.lineitem AS L
...
FROM DB2ADMIN.lineitem AS L, DB2ADMIN.supplier AS S, DB2ADMIN.partsupp AS P
...
CREATE VIEW V4 AS (
SELECT DISTINCT P.ps_partkey, P.ps_suppkey
FROM DB2ADMIN.lineitem AS L, DB2ADMIN.orders AS O, DB2ADMIN.partsupp AS P
